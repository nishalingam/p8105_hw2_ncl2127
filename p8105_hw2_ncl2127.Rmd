...
```
The `mr_trash_wheel` data has `r nrow(mr_trash_wheel)` observations and `r ncol(mr_trash_wheel)` variables and tells us about the amount in tons o and types of trash collected ranging from `r range(mr_trash_wheel$year)[1]` to `r mr_trash_wheel(pols$year)[2]`. The `prof_trash_wheel` data has `r nrow(prof_trash_wheel)` observations and `r ncol(prof_trash_wheel)` variables, ranging from years `r range(prof_trash_wheel$year)[1]` to `r range(prof_trash_wheel$year)[2]`. The `gwynnda_trash_wheel` data has `r nrow(gwynnda_trash_wheel)` observations and `r ncol(gwynnda_trash_wheel)` variables ranging from years `r range(gwynnda_trash_wheel$year)[1]` to `r range(gwynnda_trash_wheel$year)[2]`.  The total weight of trash collected by Professor Trash Wheel was `r filter(sum(prof_trash_wheel$weight_in_tons))`**.
